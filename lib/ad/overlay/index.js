...
exports.advertisement = {
  ...
  adText: {
    title: {
      text: {
        ...
        $transform (val) {
          ...
          let translation = data.getRoot().get(['dictionary', 'text', 'advertisement'])
    ...
    on: {
      click () {
        var $playerPlaying = this.cases.$playerPlaying
        var url = this.clickurl.val
        if ($playerPlaying) {
          $playerPlaying.val = false
        }
        if (url) {
