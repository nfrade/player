...
var notifyLocked = function notifyLocked () {
  var player = this.parent.parent
  if (player.locked && player.locked.val) {
    let rootdata = player.state.data.getRoot()
    let notification = rootdata.state.notification
    if (notification) {
      notification.val = rootdata.get('dictionary.video.loginRequired', 'Login required')
    }
  }
}
...
exports.overlay = {
  ...
  interaction: {
    on: {
      click () {
        if (!exports.block) {
          notifyLocked.call(this)
          this.parent.parent.toggle()
        }
      },
      dblclick () {
        this.parent.controls.exit.emit('click')
      }
    }
  },
  controls: {
